"""
Exp24b: Test 29 features (31 - Bench_per_Weight - Agility_3cone_Type_Z)
Quick LGB test first, then full ensemble if promising
"""
import pandas as pd
import numpy as np
import os
...
# 31 features then remove both
features_31 = top30_features + ['Agility_3cone_Pos_Diff']
features_29 = [f for f in features_31 if f not in ['Bench_per_Weight', 'Agility_3cone_Type_Z']]
print(f"Features: {len(features_29)}")
...
# ==========================================
# Load data and train full ensemble
# ==========================================
print("\nLoading data...")
train_df, test_df, target, cat_cols = get_data()

X_train = train_df[features_29]
y_train = target
X_test = test_df[features_29]

cat_indices = [features_29.index(c) for c in cat_cols if c in features_29]
...
print(f"\nTraining with {N_SEEDS} seeds, {N_FOLDS} folds...")
...
print(f"exp24 (30 features): CV = 0.85230")
print(f"exp24b (29 features): CV = {cv_final:.5f}")
print(f"vs exp24: {cv_final - 0.85230:+.5f}")
...
submission.to_csv(f'{EXP_DIR}/submission_29feat.csv', index=False)

# Save results
results = {
    'features': features_29,
    'n_features': 29,
    'removed': ['Bench_per_Weight', 'Agility_3cone_Type_Z'],
    'cv_lgb': cv_lgb,
    'cv_xgb': cv_xgb,
    'cv_cat': cv_cat,
...
with open(f'{EXP_DIR}/results_29feat.json', 'w') as f:
    json.dump(results, f, indent=2)

print(f"\nSubmission saved to {EXP_DIR}/submission_29feat.csv")
