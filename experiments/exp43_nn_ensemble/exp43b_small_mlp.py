"""
Exp43b: Small MLP architectures
より小さいMLPで過学習を抑える
"""
...
def train_mlp(X, y, X_test, features, hidden_layers, alpha=0.01):
    ...
    for seed in SEEDS:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
            ...
            oof[val_idx] += model.predict_proba(X_val_s)[:, 1] / N_SEEDS
            preds += model.predict_proba(X_te_s)[:, 1] / (N_FOLDS * N_SEEDS)
...
if __name__ == "__main__":
    ...
    for arch in architectures:
        oof, pred, cv = train_mlp(train_df, target, test_df, features, arch)
        results[arch] = (oof, pred, cv)

    # Summary
    print("\n" + "=" * 50)
    print("RESULTS SUMMARY")
    print("=" * 50)
    print(f"{'Architecture':<20} {'CV AUC':>10} {'Params':>10}")
    print("-" * 42)
    print(f"{'CatBoost':<20} {cv_cat:>10.5f} {'N/A':>10}")

    for arch, (_, _, cv) in sorted(results.items(), key=lambda x: -x[1][2]):
        # Rough param count: input*h1 + h1*h2 + ... + hn*1
        params = len(features) * arch[0]
        for i in range(len(arch)-1):
            params += arch[i] * arch[i+1]
        params += arch[-1]
        print(f"{str(arch):<20} {cv:>10.5f} {params:>10}")

    # Best MLP ensemble with CAT
    best_arch = max(results.keys(), key=lambda k: results[k][2])
    best_oof, best_pred, best_cv = results[best_arch]

    print(f"\nBest MLP: {best_arch} (CV={best_cv:.5f})")
    ...
    for w_cat in np.arange(0.5, 1.0, 0.05):
        w_mlp = 1.0 - w_cat
        simple = w_cat * oof_cat + w_mlp * best_oof
        logit_ens = logit_average([oof_cat, best_oof], [w_cat, w_mlp])

        s_score = roc_auc_score(target, simple)
        l_score = roc_auc_score(target, logit_ens)

        if s_score > best_simple['score']:
            best_simple = {'score': s_score, 'w': w_cat}
        if l_score > best_logit['score']:
            best_logit = {'score': l_score, 'w': w_cat}
    ...
    print(f"MLP: mean={np.mean(best_oof):.4f}, >0.9={np.mean(best_oof>0.9)*100:.1f}%")
    print(f"Correlation: {np.corrcoef(oof_cat, best_oof)[0,1]:.4f}")
