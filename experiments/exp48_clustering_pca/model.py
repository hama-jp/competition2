"""
Exp48: Unsupervised Features
- Base: Exp46 (Leak-Free Pipeline)
- New Features:
  - K-Means Clusters: k=3, 6, 12, 50 (Physical Archetypes)
  - PCA: n_components=2, 5 (Dimensionality Reduction)
- Method:
  - Unsupervised models are fitted on TRAIN fold only.
  - Valid/Test are transformed using the fitted models.
"""
import pandas as pd
import numpy as np
import os
...
EXP_DIR = 'experiments/exp48_clustering_pca'
BASE_DIR = './'

os.makedirs(EXP_DIR, exist_ok=True)

print("=" * 60)
print("Exp48: Leak-Free Pipeline + Clustering/PCA")
print("=" * 60)

# ==========================================
# Params (From Exp46/Exp45-Fixed)
# ==========================================
cat_params = {
...
# Columns for Unsupervised Learning (Physical Stats)
...
def get_data():
    ...
    # ... (Re-implementing Feature Engineering Block strictly same as exp46 for base features)
...
# ==========================================
# Main CV Loop (With TE + Unsupervised)
# ==========================================
print("\n--- Starting Leak-Free Training (Clustering/PCA) ---")
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(train_df, target)):
        ...
        X_tr_phys_imp = imputer.fit_transform(X_tr_phys) # Fit on Train
        X_va_phys_imp = imputer.transform(X_va_phys)
        X_te_phys_imp = imputer.transform(X_te_phys)
        
        # Scale
        scaler = StandardScaler()
        X_tr_phys_sc = scaler.fit_transform(X_tr_phys_imp) # Fit on Train
        X_va_phys_sc = scaler.transform(X_va_phys_imp)
        X_te_phys_sc = scaler.transform(X_te_phys_imp)
        
        # KMeans (k=3, 6, 12, 50)
        for k in [3, 6, 12, 50]:
            kmeans = KMeans(n_clusters=k, random_state=seed) # Use CV seed for variance
            ...
            # Distance to features? (Optional, maybe too many features)
            # transform() returns distance to all centroids
            # X_tr_dist = kmeans.transform(X_tr_phys_sc)
            # for i in range(k):
            #     X_tr_raw[f'Dist_Cluster_{k}_{i}'] = X_tr_dist[:, i]
            #     # ... same for va/te
            # Keeping it simple for now: Cluster ID only.
        ...
        # Base features minus TE raw cols, plus TE cols, plus new Unsupervised cols
...
results = {
    ...
    'new_features': [f'Cluster_{k}' for k in [3, 6, 12, 50]] + [f'PCA_{n}_{i}' for n in [2, 5] for i in range(n)]
}
