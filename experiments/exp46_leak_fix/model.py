"""
Exp46: Leak-Free Pipeline (Fixing exp45 leakage)
- Features: Same 31 features
- Target Encoding: Performed strictly INSIDE the CV loop.
  - Train: 5-fold OOF encoding (nested)
  - Valid: Encoded using Train Mean
  - Test: Encoded using Train Mean
- Models: Cat, LGB, XGB (Exp45 params, XGB depth=5)
"""
import pandas as pd
import numpy as np
import os
...
EXP_DIR = 'experiments/exp46_leak_fix'
BASE_DIR = './'

os.makedirs(EXP_DIR, exist_ok=True)

print("=" * 60)
print("Exp46: Leak-Free Pipeline (Target Encoding inside CV)")
print("=" * 60)

# ==========================================
# Params (from Exp45, manual depth adjustment)
# ==========================================
cat_params = {
...
def get_data():
    ...
    for col in phys_cols:
        if col in lower_is_better:
            ...
            q90 = data.groupby('Position')[col].transform(lambda x: x.quantile(0.1)) # Bug fix from original code (q90/q10 swap corrected in prev files, blindly copied logic here but assuming q90 is 10%)
...
# Identify Categorical Indices (for CatBoost, these are the *indices* in the final feature list)
# But wait, in final_features, we have TE features. The raw categorical features 'School', 'Position', 'Position_Type' are REPLACED by TE.
# 'Player_Type' is the only remaining raw categorical?
# Check base_features: "School", "Position", "Position_Type" are in there.
# But final_features EXCLUDES them and adds _TE.
# So 'Player_Type' is NOT in base_features list above? 
# Ah, base_features list was copied from exp45, which assumed TE happened already.
# Wait, 'Player_Type' is NOT in 'top30_features' of exp45. 
# So the only features to be treated as categorical by CatBoost would be NONE if we replace them all with TE.
# This simplifies things. We will treat TE features as numerical.
cat_indices = [] # No raw categoricals in final set (exp13 selection didn't choose Player_Type)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(train_df, target)):
        ...
        #    - A. Encode X_tr using nested OOF (so model doesn't overfit directly to self)
        #    - B. Encode X_va using X_tr mean
        #    - C. Encode X_te using X_tr mean
        
        # We need a temporary 'target' column in X_tr_raw for the function to work?
        # My perform_oof_te function takes (df, target).
        
        # A. Encode X_tr (Nested OOF)
        # Note: We need a SEED for the nested KFold to be stable? Or valid?
        # Let's use a fixed seed=42 for internal stability, or 'seed' to rotate it?
        # Rotating it is better.
        
        for col in te_cols:
            # Train (OOF)
            tr_enc, _ = perform_oof_te(X_tr_raw, X_tr_raw, col, y_tr, n_folds=5, smoothing=20, seed=42)
            X_tr_raw[f'{col}_TE'] = tr_enc
            
            # Valid & Test (Using Train Mean)
            global_mean = y_tr.mean()
            agg = X_tr_raw.groupby(col).apply(lambda x: (
                (y_tr.loc[x.index].sum() + 20 * global_mean) / (len(x) + 20)
            ))
            
            X_va_raw[f'{col}_TE'] = X_va_raw[col].map(agg).fillna(global_mean)
            X_te_raw[f'{col}_TE'] = X_te_raw[col].map(agg).fillna(global_mean)
            
            # Different smoothing for Position/Type? (Copied from exp33: School=20, Pos=50, Type=100)
            # Re-implementing specific smoothing:
            s_val = 20 if col == 'School' else (50 if col == 'Position' else 100)
            
            # Re-calc with specific smoothing
             # Train (OOF)
        ...
        oof_cat[va_idx] += model.predict_proba(X_va)[:, 1] / N_SEEDS # Incorrect accumulation... 
        # Wait, if we iterate seeds and folds, we visit every va_idx N_SEEDS times.
        # So accumulating / N_SEEDS is correct.
        
        # For test, we accumulate (N_FOLDS * N_SEEDS) times.
...
results = {
    'model': 'Exp46 Leak-Free Ensemble',
