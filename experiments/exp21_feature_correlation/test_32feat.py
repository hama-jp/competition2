"""
Exp21: Test 32 features (Top30 + Agility_3cone_Pos_Diff + Shuttle_Type_Z)
Quick test with LGB only
"""
import pandas as pd
import numpy as np
import os
...
# Add new features
features_32 = top30_features + ['Agility_3cone_Pos_Diff', 'Shuttle_Type_Z']
print(f"Features: {len(features_32)}")
...
X_train = train_df[features_32]
y_train = target
X_test = test_df[features_32]

cat_indices = [features_32.index(c) for c in cat_cols if c in features_32]
...
print(f"\nExp13 (30 features): CV = 0.85138, LB = 0.84524")
print(f"31 features:         CV = 0.85161")
print(f"Difference from exp13: {cv_final - 0.85138:+.5f}")
...
submission.to_csv(f'{EXP_DIR}/submission_32feat.csv', index=False)
...
with open(f'{EXP_DIR}/results_32feat.json', 'w') as f:
    json.dump(results, f, indent=2)

print(f"\nSubmission saved to {EXP_DIR}/submission_32feat.csv")
