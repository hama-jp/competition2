"""
Exp21: Test 31 features (Top30 + Agility_3cone_Pos_Diff)
Full ensemble with multiple seeds
"""
import pandas as pd
import numpy as np
import os
...
print("=" * 60)
print("Exp21: Test 31 features (Top30 + Agility_3cone_Pos_Diff)")
print("=" * 60)
...
print(f"Features: {len(features_31)}")
...
X_train = train_df[features_31]
y_train = target
X_test = test_df[features_31]

cat_indices = [features_31.index(c) for c in cat_cols if c in features_31]
...
print(f"Difference: {cv_final - 0.85138:+.5f}")
...
submission.to_csv(f'{EXP_DIR}/submission_31feat.csv', index=False)
...
with open(f'{EXP_DIR}/results_31feat.json', 'w') as f:
    json.dump(results, f, indent=2)

print(f"\nSubmission saved to {EXP_DIR}/submission_31feat.csv")
