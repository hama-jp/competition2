"""
Exp37: CatBoost Hyperparameter Tuning with Optuna
- Base: exp33 (31 features, CatBoost only)
- Goal: Find optimal CatBoost parameters to improve LB
"""
import pandas as pd
import numpy as np
import os
...
N_FOLDS = 5
N_SEEDS_TUNING = 3  # Fewer seeds for tuning
N_SEEDS_FINAL = 5   # More seeds for final model
SEEDS = [42, 2023, 101, 555, 999]
N_TRIALS = 50

EXP_DIR = '/home/user/competition2/experiments/exp37_catboost_tuning'
BASE_DIR = '/home/user/competition2'

os.makedirs(EXP_DIR, exist_ok=True)

print("=" * 60)
print("Exp37: CatBoost Hyperparameter Tuning")
print("=" * 60)
...
def objective(trial):
    ...
    scores = []
    for seed in SEEDS[:N_SEEDS_TUNING]:
...
print(f"\nBest trial:")
print(f"  Value (CV): {study.best_trial.value:.5f}")
print(f"  Params:")
...
# ==========================================
print(f"\n--- Final Training with {N_SEEDS_FINAL} seeds ---")
...
for seed_idx, seed in enumerate(SEEDS[:N_SEEDS_FINAL]):
    print(f"\n--- Seed {seed} ({seed_idx+1}/{N_SEEDS_FINAL}) ---")
    kf = StratifiedKFold(n_splits=N_FOLDS, shuffle=True, random_state=seed)
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X_train, y_train)):
        ...
        oof_cat[va_idx] += model_cat.predict_proba(X_va)[:, 1] / N_SEEDS_FINAL
        pred_cat += model_cat.predict_proba(X_test)[:, 1] / (N_FOLDS * N_SEEDS_FINAL)

    cv_cat = roc_auc_score(y_train, oof_cat * N_SEEDS_FINAL / (seed_idx + 1))
    print(f"  CAT CV: {cv_cat:.5f}")

# Final score
cv_final = roc_auc_score(y_train, oof_cat)

print("\n" + "=" * 60)
print("Results")
print("=" * 60)
print(f"Tuned CatBoost CV: {cv_final:.5f}")
print(f"\nexp33 (default params): CV = 0.85083, LB = 0.85130")
...
results = {
    ...
    'tuning_cv': float(study.best_trial.value),
