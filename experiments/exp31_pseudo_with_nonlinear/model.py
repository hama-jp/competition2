"""
Exp31: Pseudo-labeling with non-linear features (from Chatterjee analysis)
- Base: exp26 (31 features + 100 pseudo-labels)
- Add: Age_old_penalty, Agility_penalty (from exp28, exp30)
"""
import pandas as pd
import numpy as np
import os
...
EXP_DIR = '/home/user/competition2/experiments/exp31_pseudo_with_nonlinear'
BASE_DIR = '/home/user/competition2'

os.makedirs(EXP_DIR, exist_ok=True)

print("=" * 60)
print("Exp31: Pseudo-labeling with non-linear features")
print("=" * 60)

# Load exp13 results - use 31 features like exp21
...
# Features: 31 (exp21) + 2 (new non-linear) = 33
features = top30_features + ['Agility_3cone_Pos_Diff', 'Age_old_penalty', 'Agility_penalty']
print(f"Features: {len(features)}")
print(f"New features: Age_old_penalty, Agility_penalty")
...
def get_data():
    ...
    # ==========================================
    # NEW: Non-linear features from Chatterjee analysis
    # ==========================================
    # Age_old_penalty: penalty for age >= 24 (from exp28)
    data['Age_old_penalty'] = np.maximum(0, data['Age'] - 23)

    # Agility_penalty: penalty for slow agility (from exp30)
    data['Agility_penalty'] = np.maximum(0, data['Agility_3cone'] - 7.2)
...
print(f"Train pos ratio: {train_pos_ratio*100:.1f}%")

# ==========================================
# Step 1: Get initial predictions
# ==========================================
print("\n--- Step 1: Getting initial predictions ---")
...
# ==========================================
# Step 2: Select 100 pseudo-labels (balanced)
# ==========================================
print(f"\n--- Step 2: Selecting {N_PSEUDO} pseudo-labels (balanced) ---")

confidence = np.abs(pred_ensemble - 0.5)
labels = (pred_ensemble >= 0.5).astype(int)

n_pos = int(N_PSEUDO * train_pos_ratio)
n_neg = N_PSEUDO - n_pos

pos_mask = labels == 1
neg_mask = labels == 0

pos_conf = confidence.copy()
pos_conf[~pos_mask] = -1
neg_conf = confidence.copy()
neg_conf[~neg_mask] = -1

top_pos_idx = np.argsort(-pos_conf)[:n_pos]
top_neg_idx = np.argsort(-neg_conf)[:n_neg]

selected_idx = np.concatenate([top_pos_idx, top_neg_idx])
X_pseudo = X_test.iloc[selected_idx].values
y_pseudo = labels[selected_idx]

print(f"Selected: {len(selected_idx)} samples")
print(f"  Positive: {y_pseudo.sum()} ({y_pseudo.mean()*100:.1f}%)")
print(f"  Negative: {len(y_pseudo) - y_pseudo.sum()} ({(1-y_pseudo.mean())*100:.1f}%)")

# ==========================================
# Step 3: Full ensemble with pseudo-labels
# ==========================================
...
pred_lgb_final = np.zeros(len(test_df))
pred_xgb_final = np.zeros(len(test_df))
pred_cat_final = np.zeros(len(test_df))
...
for seed_idx, seed in enumerate(SEEDS):
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X_train, y_train)):
        X_tr_orig = X_train.iloc[tr_idx].values
        y_tr_orig = y_train.iloc[tr_idx].values
        X_va, y_va = X_train.iloc[va_idx].values, y_train.iloc[va_idx].values
        ...
        pred_lgb_final += model_lgb.predict_proba(X_test)[:, 1] / (N_FOLDS * N_SEEDS)
        ...
        pred_xgb_final += model_xgb.predict_proba(X_test)[:, 1] / (N_FOLDS * N_SEEDS)
        ...
        pred_cat_final += model_cat.predict_proba(X_test)[:, 1] / (N_FOLDS * N_SEEDS)

    cv_lgb = roc_auc_score(y_train, oof_lgb * N_SEEDS / (seed_idx + 1))
    cv_xgb = roc_auc_score(y_train, oof_xgb * N_SEEDS / (seed_idx + 1))
    cv_cat = roc_auc_score(y_train, oof_cat * N_SEEDS / (seed_idx + 1))
    print(f"  LGB: {cv_lgb:.5f}, XGB: {cv_xgb:.5f}, CAT: {cv_cat:.5f}")
...
pred_final = pred_lgb_final * 0.3 + pred_xgb_final * 0.3 + pred_cat_final * 0.4
...
print(f"\nexp26 (31 features): CV = 0.85175")
print(f"exp21 (no pseudo):   CV = 0.85161, LB = 0.84582")
print(f"Difference vs exp26: {cv_final - 0.85175:+.5f}")
...
results = {
    ...
    'features': features,
    'new_features': ['Age_old_penalty', 'Agility_penalty'],
    'n_pseudo': N_PSEUDO,
    'n_pos': int(y_pseudo.sum()),
    'n_neg': int(len(y_pseudo) - y_pseudo.sum()),
    'cv_lgb': float(cv_lgb),
    'cv_xgb': float(cv_xgb),
    'cv_cat': float(cv_cat),
