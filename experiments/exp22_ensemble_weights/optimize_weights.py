"""
Exp22: Optimize ensemble weights for 31 features
"""
import pandas as pd
import numpy as np
import os
...
EXP_DIR = '/home/user/competition2/experiments/exp22_ensemble_weights'
BASE_DIR = '/home/user/competition2'

os.makedirs(EXP_DIR, exist_ok=True)

print("=" * 60)
print("Exp22: Optimize Ensemble Weights")
print("=" * 60)
...
# Test weight combinations (step 0.1)
...
for w_lgb in weights:
    for w_xgb in weights:
        ...
        if w_cat < -0.01 or w_cat > 1.01:
            continue
        w_cat = max(0, min(1, w_cat))

        oof_ensemble = oof_lgb * w_lgb + oof_xgb * w_xgb + oof_cat * w_cat
        cv = roc_auc_score(y_train, oof_ensemble)
...
print(f"{'LGB':>6} {'XGB':>6} {'CAT':>6} {'CV':>10}")
print("-" * 35)
for r in results[:10]:
    print(f"{r['w_lgb']:6.1f} {r['w_xgb']:6.1f} {r['w_cat']:6.1f} {r['cv']:10.5f}")

print(f"\nCurrent weights (0.3, 0.3, 0.4):")
cv_current = roc_auc_score(y_train, oof_lgb * 0.3 + oof_xgb * 0.3 + oof_cat * 0.4)
print(f"  CV: {cv_current:.5f}")

print(f"\nBest weights: LGB={best_weights[0]:.1f}, XGB={best_weights[1]:.1f}, CAT={best_weights[2]:.1f}")
print(f"  CV: {best_cv:.5f}")
print(f"  Improvement: {best_cv - cv_current:+.5f}")

# ==========================================
# Create submission with best weights
# ==========================================
pred_best = pred_lgb * best_weights[0] + pred_xgb * best_weights[1] + pred_cat * best_weights[2]
...
results_summary = {
    ...
    'cv_xgb': cv_xgb,
    'cv_cat': cv_cat,
    'cv_current_weights': cv_current,
    'current_weights': [0.3, 0.3, 0.4],
    'best_weights': list(best_weights),
    'best_cv': best_cv,
    'improvement': best_cv - cv_current,
