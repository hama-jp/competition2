"""
Exp04: Pseudo Labeling
- Use high-confidence predictions on test data as additional training data
...
PSEUDO_THRESHOLD = 0.15  # Use samples with pred < 0.15 or pred > 0.85

BASE_DIR = '/home/user/competition2'
EXP_DIR = os.path.dirname(os.path.abspath(__file__))

# Load best params from exp03
EXP03_DIR = '/home/user/competition2/experiments/exp03_tabnet'
...
# Data Preparation
# ==========================================
def get_data(include_pseudo=False, pseudo_df=None, pseudo_target=None):
    ...
    if include_pseudo and pseudo_df is not None and pseudo_target is not None:
        print(f"Adding {len(pseudo_df)} pseudo-labeled samples...")
        train_df = pd.concat([train_df, pseudo_df], ignore_index=True)
        target = pd.concat([target, pseudo_target], ignore_index=True)

    return train_df, test_df, target, features, cat_indices

# ==========================================
# Training Functions (with best params from exp03)
# ==========================================
...
# ==========================================
# Main Pipeline with Pseudo Labeling
# ==========================================
if __name__ == '__main__':
    print("="*50)
    print("Exp04: Pseudo Labeling")
    print("="*50)

    # Round 1: Train without pseudo labels
    ...
    round1_cv = -res.fun
    print(f"Round 1 CV: {round1_cv:.5f}")

    # Create ensemble predictions for pseudo labeling
    ensemble_pred = (pred_lgb * best_w[0]) + (pred_xgb * best_w[1]) + (pred_cat * best_w[2])

    # Select high confidence samples
    high_conf_positive = ensemble_pred > (1 - PSEUDO_THRESHOLD)  # > 0.85
    high_conf_negative = ensemble_pred < PSEUDO_THRESHOLD  # < 0.15

    pseudo_mask = high_conf_positive | high_conf_negative
    print(f"\nHigh confidence samples: {pseudo_mask.sum()} ({pseudo_mask.sum()/len(test_df)*100:.1f}%)")
    print(f"  Positive (>0.85): {high_conf_positive.sum()}")
    print(f"  Negative (<0.15): {high_conf_negative.sum()}")
    ...
    pseudo_labels = pd.Series((ensemble_pred[pseudo_mask] > 0.5).astype(float))

    # Re-process data with pseudo labels
    train_df2, test_df2, target2, features2, cat_indices2 = get_data(
        include_pseudo=True,
        pseudo_df=pseudo_test[features],
        pseudo_target=pseudo_labels
    )

    print(f"Total training samples: {len(train_df2)}")

    oof_lgb2, pred_lgb2 = train_lgb(train_df2, target2, test_df2, features2)
    oof_xgb2, pred_xgb2 = train_xgb(train_df2, target2, test_df2, features2)
    oof_cat2, pred_cat2 = train_cat(train_df2, target2, test_df2, features2, cat_indices2)

    # Final ensemble
    final_preds = (pred_lgb2 * best_w[0]) + (pred_xgb2 * best_w[1]) + (pred_cat2 * best_w[2])

    # Use original OOF for CV (without pseudo labels)
    original_train_size = 2781  # Original train size
    oof_lgb2_orig = oof_lgb2[:original_train_size]
    oof_xgb2_orig = oof_xgb2[:original_train_size]
    oof_cat2_orig = oof_cat2[:original_train_size]
    target_orig = target2[:original_train_size]

    final_oof = (oof_lgb2_orig * best_w[0]) + (oof_xgb2_orig * best_w[1]) + (oof_cat2_orig * best_w[2])
    round2_cv = roc_auc_score(target_orig, final_oof)

    print(f"\n{'='*50}")
    print(f"Round 1 CV: {round1_cv:.5f}")
    print(f"Round 2 CV (with pseudo): {round2_cv:.5f}")
    print(f"Improvement: {round2_cv - round1_cv:+.5f}")
    print(f"{'='*50}")
    ...
    # Also save round 1 submission
    submission_r1 = pd.read_csv(os.path.join(BASE_DIR, 'sample_submission.csv'))
    submission_r1['Drafted'] = ensemble_pred
    submission_r1.to_csv(os.path.join(EXP_DIR, 'submission_round1.csv'), index=False)

    results = {
        'experiment': 'exp04_pseudo_labeling',
        'timestamp': datetime.now().isoformat(),
        'round1_cv': round1_cv,
        'round2_cv': round2_cv,
        'pseudo_samples': int(pseudo_mask.sum()),
        'pseudo_positive': int(high_conf_positive.sum()),
        'pseudo_negative': int(high_conf_negative.sum()),
    }

    with open(os.path.join(EXP_DIR, 'results.json'), 'w') as f:
        json.dump(results, f, indent=2)

    np.save(os.path.join(EXP_DIR, 'oof_final.npy'), final_oof)
